...
export default function Home() {
  ...
  return (
    <main className="min-h-screen min-w-full bg-slate-900 p-6">
      <div
        className="m-w-64 m-auto p-1
                         md:m-w-96"
      >
        <input
          type="text"
          className="w-full min-h-9 block m-auto text-center bg-slate-800"
        ></input>
        <button id="5min-button" onClick={startTimer}>
          {" "}
          5 min
        </button>
        {/* <div className='grid '>
      ...
      {!timer.notified && <Timer timer={timer} />}
    </main>
  );
}
